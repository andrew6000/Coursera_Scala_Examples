package functional.design.week.two.streams
...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] = {
    //b.legalNeighbors map { case (b, m) => (b, m :: history) } toStream
     val pf: PartialFunction[(Block, Move), (Block, List[Move])] = {

      case (b,m)  => (b,m :: history)
    }
    b.legalNeighbors.collect(pf).toStream
  }



  /*def getHistory(history: List[Move]):List[Move]= {
  ...
  def newNeighborsOnly(neighbors: Stream[(Block, List[Move])],
    ...
    val pf: PartialFunction[(Block, List[Move]), (Block, List[Move])] = {

      case (b,m) if(!(explored contains(b)))  => (b,m)
    }
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    case (b, ms) #:: rest => {
      lazy val more = newNeighborsOnly(neighborsWithHistory(b, ms), explored + b)
