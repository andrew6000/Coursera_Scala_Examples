...
object MergeSort2 {
  ...
  def parMergeSortWithTask(xs: Array[Int], maxDepth: Int): Unit = {
    ...
    // Without the merge step, the sort phase parallelizes almost linearly.
    // This is because the memory pressure is much lower than during copying in the third step.
    def sortWithTask(from: Int, until: Int, depth: Int): Unit = {
      if (depth == maxDepth) {
        quickSort(xs, from, until - from)
      } else {
        val mid = (from + until) / 2
        val right = task {
          sortWithTask(mid, until, depth + 1)
        }
        sortWithTask(from, mid, depth + 1)
        right.join()
    ...
    // However, most Intel processors have a dual-channel memory controller,
    // so parallel copying has very small performance benefits.
    def copyWithTask(src: Array[Int], target: Array[Int], from: Int, until: Int, depth: Int): Unit = {
      if (depth == maxDepth) {
        Array.copy(src, from, target, from, until - from)
      } else {
        val mid = from + ((until - from) / 2)
        val right = task {
          copyWithTask(src, target, mid, until, depth + 1)
        }
        copyWithTask(src, target, from, mid, depth + 1)
        right.join()
      }
    }
    if (maxDepth % 2 != 0) {
      copyWithTask(ys, xs, 0, xs.length, 0)
    }
  }
  ...
  def main(args: Array[String]) {
    ...
    val partimeWithTask = standardConfig setUp {
      ...
      parMergeSortWithTask(xs, maxDepth)
    }

    println(s"\nfork/join time without task: $partime ms")
    println(s"speedup: ${seqtime / partime}")

    println(s"\nfork/join time with task: $partimeWithTask ms")
